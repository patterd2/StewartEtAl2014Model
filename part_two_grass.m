function [fn_change_in_grass]=part_two_grass()
%FUNCTION - Calculate growth rate of species 1= grass
...
% subroutines
global fn_change_in_grass
global grass_resource
...
for column=1:fieldsize  
    for row=1:fieldsize
        ...
        for res=1:randp
            actual_res(res)=grass_resource(row,column,res); %makes calculations easier to follow
        ...
        switch lower (method)   %NB Matlab switch does not break through - i.e. if case=1 and case=2, only case=1 is executed
            case 1  %water drought - insufficient water to maintain current biomass               
                ...
                grass_resource(row,column,1)=0; %All water is used, no nitrogen is used
            ...
            case 2  %nitrogen drought - insufficient nitrogen to maintain current biomass
                ...
                grass_resource(row,column,2)=0; %All nitrogen is used
                grass_resource(row,column,1)=actual_res(1)-field_species(row,column,plant)*Maintenance(plant,1);     
            ...
            case 3  %Biomass increases - either water or nitrogen limits new growth
                ...
                for res=1:randp
                    grass_resource(row,column,res)=actual_res(res)-bio_mass*Maintenance(plant,res)-growth_rate(row,column,plant)*Efficiency(plant,res); %calculate resource use
                end              
            otherwise
                disp ('error in part two grass - resource use calculation') %Can't imagine why you'd ever see this error, but better safe than sorry
        ...
        for res=1:randp
            if (grass_resource(row,column,res)<0) %error checking section - ensure grass_resource hasn't taken a negative value
                if ((grass_resource(row,column,res)<-0.000001)||(grass_resource(row,column,res)>0.000001))
                    disp ('correction made in part two grass - resource less than 0')      %enable breakpoint here for debugging - this error SHOULD NOT happen
                end
                grass_resource(row,column,res)=0; %but, there can be very small variances in grass resource when all resource is used
            end
        end
    end    %end of row loop
